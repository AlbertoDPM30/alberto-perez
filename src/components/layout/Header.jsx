...
export default function Header({ tags = {} }) {
  ...
  return (
    <header className="h-[3rem] md:h-[5rem] flex items-center fixed top-0 left-0 w-full z-50">
      ...
      <nav className="relative z-40 w-[100dvw] md:hidden flex flex-col items-end justify-center px-4">
        <button
          onClick={() =>
            setShowNav((prev) => (prev === "hidden" ? "flex" : "hidden"))
          }
      ...
      {/* DESKTOP */}
      <nav className="relative z-40 md:flex hidden">
